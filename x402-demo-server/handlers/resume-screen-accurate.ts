 ...
 * Accurate resume screening using Groq LLM API
 */
export async function handleResumeScreenAccurateRequest(c: Context) {
  const startTime = Date.now();
  try {
    console.log('✓ PAYMENT VERIFIED - POST /resume-screen-accurate handler executing');
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          try {
            ...
            const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
              ...
              body: JSON.stringify({
                ...
                messages: [
                  {
                    role: 'system',
                    content: 'Analyze the resume text against the target role. Return ONLY a valid JSON object matching this schema: { "name": string, "match_score": number, "role": string, "key_skills": string[] }. Do not wrap in markdown codeblocks (e.g. no ```json). Do not explain.',
                  },
                  {
                    role: 'user',
                    content: `Target role: ${task_description}\n\nResume text:\n${extracted_text.substring(0, 8000)}`,
                  },
                ],
                temperature: 0.1,
              }),
            ...
            if (response.ok) {
              ...
              try {
                // Strip code block formatting if LLM includes it
      ...
      if (!success) {
        console.warn(`Fallback active for candidate processing of file: ${file.filename}`);
      }
    ...
    console.error('Error in resume-screen-accurate handler:', error);
